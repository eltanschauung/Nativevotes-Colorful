...
public int Native_Display(Handle plugin, int numParams)
{
	...
	if (count <= 0)
	{
		PrintToServer("[Nativevotes] Skipping vote due to invalid client count (%d).", count);
		return 0;
	}

	if (count > MaxClients)
	{
		PrintToServer("[Nativevotes] Skipping vote due to invalid client count (%d > MaxClients %d).", count, MaxClients);
		return 0;
	}
	...
	int[] validClients = new int[count];
	int validCount = 0;
	for (int i = 0; i < count; i++)
	{
		...
		validClients[validCount++] = client;
	}
	...
	if (!StartVote(vote, validCount, validClients, GetNativeCell(4), flags))
	{
		return 0;
	}
