		...
		case MenuAction_VoteEnd:
		{
			...
			else
			{
				...
				switch (g_voteType)
				{
					...
					case burn:
					{
							CPrintToChatAllEx(g_voteClient[VOTE_CLIENTID], "%t", "Set target on fire", "_s", g_voteInfo[VOTE_NAME]);
					...
					case slay:
					{
							CPrintToChatAllEx(g_voteClient[VOTE_CLIENTID], "%t", "Slayed player", g_voteInfo[VOTE_NAME]);
