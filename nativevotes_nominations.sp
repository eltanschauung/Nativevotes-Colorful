...
char g_szRandomNominateMap[MAXPLAYERS + 1][PLATFORM_MAX_PATH];
...
public void OnPluginStart()
{
	...
	{
		g_hRandomNominateTimer[i] = INVALID_HANDLE;
...
void QueueRandomNomination(int client)
{
	...
	for (int i = 0; i < g_MapList.Length; i++)
	{
		...
		if (!IsMapEnabledForNomination(map))
		{
			continue;
		}
	...
	{
		delete candidates;
		CReplyToCommand(client, "[{lightgreen}Nominations\x01] No eligible maps to nominate.");
		return;
	}
	...
	char displayName[PLATFORM_MAX_PATH];
	GetMapDisplayName(map, displayName, sizeof(displayName));
	strcopy(g_szRandomNominateMap[client], sizeof(g_szRandomNominateMap[]), map);
	g_iRandomNominateUserId[client] = GetClientUserId(client);
	g_hRandomNominateTimer[client] = CreateTimer(RANDOM_NOMINATION_DELAY, Timer_RandomNominate, client);

	CReplyToCommand(client, "[{lightgreen}Nominations\x01] Random map: {green}%s", displayName);
	CReplyToCommand(client, "[{lightgreen}Nominations\x01] Use {green}!nr\x01 again within 5 seconds to roll for another map instead.");
}
...
public Action Timer_RandomNominate(Handle timer, any client)
{
	...
	g_hRandomNominateTimer[index] = INVALID_HANDLE;

	if (g_iRandomNominateUserId[index] != GetClientUserId(index))
	{
		g_szRandomNominateMap[index][0] = '\0';
		return Plugin_Stop;
	}
	if (g_szRandomNominateMap[index][0] == '\0')
	{
		return Plugin_Stop;
	}

	AttemptNominate(index, g_szRandomNominateMap[index], sizeof(g_szRandomNominateMap[]), false);
	g_szRandomNominateMap[index][0] = '\0';
	return Plugin_Stop;
}
...
void BuildRandomNominationList()
{
	...
	for (int i = candidates.Length - 1; i > 0; i--)
	{
		...
		char other[PLATFORM_MAX_PATH];
		candidates.GetString(i, current, sizeof(current));
		candidates.GetString(swapIndex, other, sizeof(other));
		candidates.SetString(i, other);
		candidates.SetString(swapIndex, current);
	}

	int limit = candidates.Length < RANDOM_NOMINATION_MENU_SIZE ? candidates.Length : RANDOM_NOMINATION_MENU_SIZE;
	for (int i = 0; i < limit; i++)
	{
		candidates.GetString(i, map, sizeof(map));
...
bool IsMapEnabledForNomination(const char[] map)
{
	int status;
	return g_MapTrie.GetValue(map, status)
		&& (status & MAPSTATUS_DISABLED) == 0;
}
...
void RefreshRecentlyPlayedMapStatuses()
{
	ClearRecentlyPlayedMapStatuses();
	if (!g_ConVars[excludeold].BoolValue || g_MapTrie == null)
	{
		return;
	}
	...
	{
		excludeMaps.GetString(i, map, sizeof(map));
		if (FindMap(map, resolvedMap, sizeof(resolvedMap)) == FindMap_NotFound)
		{
			strcopy(resolvedMap, sizeof(resolvedMap), map);
		}
